function zz_DomeProfiles_3D_Normed();
...
for i = 2;
    figure(i)
    colormap jet
    set(gcf,'position',[740   149   789   728])
    cbar = colorbar();
    set(gcf,'color','w');
    set(gca,'Tickdir','out')
    %set(gca,'Xtick',[-2 -1 0 1 2]/R)
    %set(gca,'Ytick',[-2 -1 0 1 2]/R)
    %set(gca,'Ztick',[0.6 0.8 1.0 1.2 1.4 1.6]/R)
    set(gca,'linewidth',1.5,'fontsize',16)
    xlabel('x'' / R','fontsize',16,'rot',0)
    ylabel('y'' / R','fontsize',16,'rot',0)
    zlabel({'z''','R'},'fontsize',16,'rot',0)
    if any( i == [1 3])
        ylabel(cbar,'e_e' ,'rot',0,'fontsize',16)
    else
        %ylabel(cbar,'Z' ,'rot',0,'fontsize',16)
        axis([-.4 .4 -.4 .4 0.22 .55])
        %set(cbar,'YTick',[0.6 0.8 1.0 1.2 1.4 1.6]/R)
        %set(cbar,'Position',[0.939423558897243   0.202182410423453   0.020050125313283   0.671465798045603])
        %set(cbar,'position',
        %set(cbar,'Position',[0.948641740599915-.05   0.210000000000000   0.033798056611745   0.665000000000000]')
    end
    if i == 2 || i ==4
        %caxis([0.6 1.6]/R)
    end
    view( -61,  9.2644 )
    grid on
    set(gca,'GridLineStyle','-','GridColor',[0.15 0.15 0.15])
