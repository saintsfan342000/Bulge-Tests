...
for i = stages
    ...
    for j = s+1:length(C)-s;
        cfilt = [cfilt mean([C(j-s:j+s)]) ];
        dfilt = [dfilt mean([d(j-s:j+s)]) ];
    end
    
    cfilt = [C(1) cfilt C(end)];
    dfilt = [d(1) dfilt d(end)];
    
    
    %a(k) = plot(dfilt,cfilt,'color',c{k},'linewidth',2);
    
    a(k)=plot(-dfilt,cfilt,'color',c{k},'linewidth',2);
    hold on
    
    %xlswrite('../../AA_Paper/StrainProfiles.xlsx',[-dfilt' cfilt'],sprintf('Mark %d, Stg %d',k-1,i))
...
set(gcf,'color','w')
L=legend(fliplr(a),fliplr({'0','1','2','3','4','5','6','7'}));
 set(L,'location','southeastoutside')
ylabel('e_e','fontsize',16);
