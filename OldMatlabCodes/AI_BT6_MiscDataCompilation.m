% Special for BT-6 since the tr/ro strain ratio switched from >1 to < 1
% Determined by inspection that 
...
STLP = load( [path 'STLP.dat'] );
    % Error in STLP for BT-3.  It needs to be multiplied by 10
    if lstring{i} == 'BT-3'
        L = (STLP(:,3)-STLP(1,3))*10; 
    else
        L = STLP(:,3)-STLP(1,3); 
    end
    
    P = STLP(:,4); time = STLP(:,2);
...
%(7)Mean ex (8)Mean ey (9)Mean ey/ex (10)std(ey/ex) (11) Dome Heigh[in] (12)Thickness (13)Change-Thickness
    ...
    R = D(:,1) ; Z = D(:,11) ; t = D(:,12) ; dt = D(:,13) ; ex = D(:,7) ; ey = D(:,8) ; phi=D(:,6);
    e1 = D(:,2) ;  e2 = D(:,3) ;
...
% Critical pressure at which I think ratio may have really started to switch
Pc =  1114.7;
Ploc =  find(P >= Pc);
Ploc = Ploc(1);
% Critical bulge height at which I think ratio may have really started to switch
Zc = 1.3;
Zloc =  find(Z >= Zc);
Zloc = Zloc(1);

% Plastic strain points
% SPECIFY HERE WHETHER WE WANT TO USE PRESSURE CRITICAL OR HEIGHT CRITICAL
interppoints = linspace(0.02,e1p(Zloc),4);
%interppoints = linspace(.08,.12,4);
...
%axis([0 .1 0 .1])
%set(gca,'Xlim',[0 .1])
text(interppoints(end),e1e2p(end),sprintf('m = %.3f',E(1)),'color',c{i})
xlabel('e^p_x','fontsize',14)
ylabel('e^p_y    ','rot',0,'fontsize',14)
set(gca,'linewidth',1.5)
set(gca,'Tickdir','out')
%text(interppoints(end),e1e2p(end),{'','',sprintf('%.3f',interppoints(end)),'',sprintf('Z_c = %.1f',Zc)},'color',[.5 .5 .5],'VerticalAlignment','top')
